...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/09/17 12:59:56 by jpinyot           #+#    #+#             */
/*   Updated: 2018/09/17 13:01:16 by jpinyot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	ft_putstr("\x1B[91mInvalid xor parameters at line ");
	ft_putnbr(l);
	if (sel == 0)
...
t_line		*ft_add_xor(char *l, int cnt, int j, int n_line)
{
	...
	if ((i = ft_strcmp_index_jmp(&l[j], "xor")) == -1 || l[i] == 0)
		ft_error_order(n_line, 0, "xor", "xor\tr2,r2,r2");
	i += j;
	line = ft_newline(NULL, 8, l, 2);
	if (l[i] == DIRECT_CHAR)
