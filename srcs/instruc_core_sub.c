...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/09/27 20:15:11 by mrodrigu          #+#    #+#             */
/*   Updated: 2018/09/27 20:15:45 by mrodrigu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		instruc_core_sub(t_pc *pc)
{
	...
	if ((0xFC & ocp) == 0x54 && (reg1 = g_mem[(pos + 2) % MEM_SIZE] - 1) < REG_NUMBER && (reg2 = g_mem[(pos + 3) % MEM_SIZE] - 1) < REG_NUMBER && (reg3 = g_mem[(pos + 4) % MEM_SIZE] - 1) < REG_NUMBER)
		...
		pc->pc = (pc->pc + 1 + 1 + get_size_arg(ocp, 0, 1) + get_size_arg(ocp, 1, 1) + get_size_arg(ocp, 2, 1)) % MEM_SIZE;//sub, ocp, a1, a2, a3
}
